...
class LifecycleConfig:
    ...
    lambda_eq_per_year: float = 0.025
    lambda_fire_per_year: float = 0.20
    ...
    maint_costs: tuple[float, float, float] = (0.0, 0.25, 0.6)
...
def _evaluate_population_torch(
    ...
    *,
    ...
    max_repair_duration = max(
        [float(x) for x in cfg.repair_durations_years] + [0.0])
    repair_slot_count = max(
        2,
        int(math.ceil(max_repair_duration / max(decision_dt, 1.0e-12))) * 2 + 2,
    )
    repair_remaining = torch.zeros(
        (repair_slot_count, batch), device=device, dtype=torch.float32)
    repair_rates = torch.zeros_like(repair_remaining)
    ...
    for s in range(steps):
        ...
        if is_year_boundary:
            ...
            if y < years:
                ...
                def _apply_event(intensity):
                    nonlocal f, cost, npv, risk, repair_remaining, repair_rates
                    has = intensity > 0.0
                    ...
                    f = torch.where(has, f_after_damage, f)
                    ...
                    for slot in range(repair_slot_count):
                        ...
                        fill = pending & empty
                        if bool(torch.any(fill).item()):
                            repair_remaining[slot] = torch.where(
                        ...
                        if not bool(torch.any(pending).item()):
                            break
                    if bool(torch.any(pending).item()):
                        last = repair_slot_count - 1
                        repair_remaining[last] = torch.where(
                            pending,
                            torch.maximum(repair_remaining[last], duration),
                            repair_remaining[last],
                        )
        ...
        repair_used = torch.minimum(repair_remaining, dt_t)
        repair_delta = (repair_rates * repair_used).sum(dim=0)
        repair_remaining = torch.clamp(repair_remaining - repair_used, min=0.0)
        repair_rates = torch.where(
            repair_remaining > 1.0e-12,
            repair_rates,
...
def _plot_iterations(
    ...
    *,
    ...
    ax_f.plot(t_base, f_base, color="tab:red", linewidth=_lw(2.0),
              linestyle="-", label="Fixed-rule", zorder=30, alpha=1.0,
    ...
    ax_f.plot(t_best, f_best, color="tab:green", linewidth=_lw(2.0),
              label="RL-optimized", zorder=31, alpha=1.0,
              path_effects=_KEY_LINE_EFFECT)
    ax_c.plot(t_best, cc_best, color="tab:green", linewidth=_lw(2.0),
              label="RL-optimized", zorder=31, alpha=1.0,
              path_effects=_KEY_LINE_EFFECT)
    ...
    ax_compare.plot(t_base, f_base, color="tab:red",
                    linewidth=_lw(2.0), linestyle="-", label="Fixed-rule",
                    alpha=1.0, zorder=30, path_effects=_KEY_LINE_EFFECT)
    ...
    if _PLOT_TITLES:
        ax_compare.set_title("Fixed-rule vs RL-optimized Policy",
        ...
        with plt.rc_context(_tex_rc_params()):
            ...
            if "reward" in out_path.name:
                if out_path.name.endswith("reward_per_episode.png"):
                    ...
                    ax.set_yticks([-35.0, -30.0, -25.0, -20.0, -15.0, -10.0, -5.0])
...
def _case_run_configs(root: Path) -> list[dict[str, Any]]:
    ...
    cases: list[dict[str, Any]] = [
        {
            "name": "case1_baseline",
            "lambda_eq_per_year": 1.0 / 40.0,
            "lambda_fire_per_year": 0.20,
            "w_lr": 1.0 / 3.0,
            "w_risk": 1.0 / 3.0,
            "w_cost": 1.0 / 3.0,
        },
        {
            ...
            "lambda_eq_per_year": 1.0 / 80.0,
            "lambda_fire_per_year": 0.40,
            "w_lr": 1.0 / 3.0,
            "w_risk": 1.0 / 3.0,
            "w_cost": 1.0 / 3.0,
        },
        {
            ...
            "lambda_eq_per_year": 2.0 / 40.0,
            "lambda_fire_per_year": 0.10,
            "w_lr": 1.0 / 3.0,
            "w_risk": 1.0 / 3.0,
            "w_cost": 1.0 / 3.0,
        },
